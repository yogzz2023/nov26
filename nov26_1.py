...
class CVFilter:
    def __init__(self):
        ...
        self.gate_threshold = 90.21  # 95% confidence interval for Chi-squared distribution with 3 degrees of freedom
...
class KalmanFilterGUI(QWidget):
    def __init__(self):
        ...
        self.plot_controls_collapsed = False  # Start with plot controls expanded
    ...
    def initUI(self):
        ...
        # Plot Controls Toggle Button
        self.plot_controls_button = QPushButton("Plot Controls")
        self.plot_controls_button.clicked.connect(self.toggle_plot_controls)
        plot_controls_layout.addWidget(self.plot_controls_button)
    ...
    def toggle_plot_controls(self):
        self.plot_controls_collapsed = not self.plot_controls_collapsed
        for i in range(1, self.plot_tab.layout().count() - 1):  # Skip the first and last widgets (button and plot)
            widget = self.plot_tab.layout().itemAt(i).widget()
            if widget:
                widget.setVisible(not self.plot_controls_collapsed)
        self.adjustSize()
    ...
    def plot_ppi(self, tracks, plot):
        ...
        for track in tracks:
            ...
            azimuths = [m[0][1] for m in measurements]  # Azimuth in degrees
            ranges = [m[0][0] for m in measurements]  # Range

            # Convert azimuths to radians for polar plot
            azimuths_rad = np.radians(azimuths)

            # PPI plot (polar plot)
            plot.plot(azimuths_rad, ranges, pen=None, symbol='o', symbolSize=self.marker_size, symbolBrush=self.plot_color, name=f"Track {track['track_id']} PPI")

        plot.setLabel('left', 'Range')
        plot.setLabel('bottom', 'Azimuth (radians)')
